...
function makeSchedule(
    id: string,
    intervalMs: number,
    run?: (ctx: DomainContext) => Promise<void>,
): DomainSchedule {
    return {
        id,
        name: id,
...
describe("Scheduler", () => {
    ...
    describe("unregisterDomain", () => {
        ...
        test("does not affect other domains", async () => {
            ...
            scheduler.registerSchedule(
                "remove",
                makeSchedule("s1", 1000, () => Promise.resolve()),
            );
    ...
    describe("tick", () => {
        ...
        test("emits scheduleRun event on success", async () => {
            ...
            scheduler.registerSchedule(
                "d",
                makeSchedule("s1", 0, () => Promise.resolve()),
            );
            await scheduler.tick();

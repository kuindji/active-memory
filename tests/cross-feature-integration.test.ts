...
describe('Topic + User domain coexistence', () => {
  ...
  beforeEach(async () => {
    ...
    await userCtx.graph.relate(userFactId, 'about_user', 'user:test-user', { domain: USER_DOMAIN_ID })
  ...
  test('memories owned by different domains are independently searchable', async () => {
    ...
    const bothResults = await engine.search({ mode: 'graph', domains: [USER_DOMAIN_ID, TOPIC_DOMAIN_ID] })
    ...
    expect(bothResults.entries.length).toBe(userResults.entries.length + topicResults.entries.length)
  })
  ...
  test('merge and consolidation schedules do not interfere', async () => {
    ...
    await userCtx.graph.relate(mem2, 'about_user', 'user:test-user', { domain: USER_DOMAIN_ID })
    ...
    const topicSearch = await topicCtx.search({ text: 'TypeScript programming language', tags: [TOPIC_TAG] })
    const statuses = topicSearch.entries.map(e => e.domainAttributes[TOPIC_DOMAIN_ID]?.status)
    ...
    const summary = allUserMemories.find(m => m.content === 'User knows TypeScript and enjoys hiking.')
...
describe('Visibility with built-in domains', () => {
  ...
  test('getNodeEdges on user node filters edges from non-visible domains', async () => {
    ...
    expect(sourceIds.some(id => id === userFactId || id.endsWith(userFactId.replace('memory:', '')))).toBe(true)

    // Notes-owned memory edge should NOT be present
    expect(sourceIds.some(id => id === notesMemoryId || id.endsWith(notesMemoryId.replace('memory:', '')))).toBe(false)
  })
...
describe('End-to-end: user + topic + context', () => {
  ...
  test('full lifecycle: ingest, link, search, consolidate, merge', async () => {
    ...
    expect(incomingMemIds.some(id => id === tsFact || id.includes(tsFact.replace('memory:', '')))).toBe(true)
    expect(incomingMemIds.some(id => id === rustFact || id.includes(rustFact.replace('memory:', '')))).toBe(true)

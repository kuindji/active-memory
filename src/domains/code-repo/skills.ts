...
const codeRepoCapture: DomainSkill = {
  ...
  description: 'Tells external agents how to write curated code repo knowledge — decisions, rationale, clarifications, direction — with classification metadata and audience tags (technical/business)',
  scope: 'external',
  writes: true,
}
...
const codeRepoQuery: DomainSkill = {
  ...
  description: 'Tells external agents how to search code repo knowledge by classification, audience, and entity, traverse the architecture graph, and use buildContext with audience filtering',
  scope: 'external',
}

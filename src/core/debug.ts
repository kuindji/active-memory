...
function wrapLLMAdapter(adapter: LLMAdapter, debug: DebugTools, scope: string): LLMAdapter {
  ...
  if (adapter.extractStructured) {
    wrapped.extractStructured = (text: string, schema: string, prompt?: string): Promise<unknown[]> =>
      debug.time(`${scope}.extractStructured`, () => adapter.extractStructured!(text, schema, prompt), {
        chars: text.length,
  ...
  if (adapter.rerank) {
    wrapped.rerank = (query: string, candidates: { id: string; content: string }[]): Promise<string[]> =>
  ...
  if (adapter.synthesize) {
    wrapped.synthesize = (
      ...
      debug.time(`${scope}.synthesize`, () => adapter.synthesize!(query, memories, tagContext), {
        chars: query.length,
        memories: memories.length,

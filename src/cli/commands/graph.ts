...
const graphCommand: CommandHandler = async (engine, parsed) => {
    ...
    if (!subcommand) {
        return {
            output: { error: "Subcommand is required: edges, relate, unrelate, traverse" },
            exitCode: 1,
        };
    }
    ...
    if (subcommand === "relate") {
        ...
        const attrs =
            attr && typeof attr === "object" ? (attr as Record<string, unknown>) : undefined;
    ...
    if (subcommand === "traverse") {
        ...
        if (!edgesFlag)
            return { output: { error: "--edges is required for traverse" }, exitCode: 1 };

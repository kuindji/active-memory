    ...
    return async function processInboxBatch(
        ...
        await context.debug.time(
            ...
            async () => {
                ...
                const { processable: processableEntries, decomposedParents } =
                    await context.debug.time(
                        "kb.inbox.decompose",
                    ...
                    if (decomposedParents.length > 0) {
                        ...
                        for (const parent of decomposedParents) {
                            const classification =
                                parentClassificationMap.get(parent.memory.id) ?? "fact";
...
async function batchClassify(
    ...
    if (classifyLlm.extractStructured) {
        try {
            ...
            logWarn("kb.inbox.classify.extractStructured", error);
        }
        ...
        try {
            ...
            logWarn("kb.inbox.classify.generate", error);
        }
    }

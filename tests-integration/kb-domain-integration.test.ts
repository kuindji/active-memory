...
describe('KB inbox processing with classification and topic linking (real)', () => {
  ...
  test('ingest multiple entries with different classifications', async () => {
    ...
    const definitions = await ctx.getMemories({ tags: [KB_TAG], attributes: { classification: 'definition' } })
    const references = await ctx.getMemories({ tags: [KB_TAG], attributes: { classification: 'reference' } })
    const concepts = await ctx.getMemories({ tags: [KB_TAG], attributes: { classification: 'concept' } })
    ...
    console.log(`[PASS] Definitions: ${definitions.length}, References: ${references.length}, Concepts: ${concepts.length}`)
  })
})
...
describe('Supersession detection (real)', () => {
  ...
  test('new corrected fact creates supersedes edge on old one', async () => {
    ...
    await engine.ingest(
      'The maximum size of a single HTTP header in most web servers is 4KB',
      {
        domains: [KB_DOMAIN_ID],
        metadata: { classification: 'fact' },
      },
    ...
    const firstFacts = await ctx.getMemories({ tags: [KB_TAG], attributes: { classification: 'fact' } })
    ...
    if (supersededEdges.length > 0) {
      console.log(`[PASS] Supersession detected! New fact supersedes ${supersededEdges.length} old fact(s)`)
      ...
      if (oldFacts.length > 0) {
        console.log(`  Old fact marked superseded: "${oldFacts[0].content.slice(0, 80)}..."`)
      }
    } else {
    ...
    const allFacts = await ctx.getMemories({ tags: [KB_TAG], attributes: { classification: 'fact' } })
  ...
  test('related entries get linked with relationship type', async () => {
    ...
    if (relatedEdges.length > 0) {
      ...
      if (relKnowledgeEdge) {
        ...
        const rel = typeof edgeData.relationship === 'string' ? edgeData.relationship : 'unknown'
...
describe('buildContext with sectioned output (real)', () => {
  ...
  test('buildContext returns structured sections excluding superseded entries', async () => {
    ...
    await ctx.writeMemory({
      content: 'A mutex (mutual exclusion) is a synchronization primitive that allows only one thread to access a shared resource at a time',
    ...
    // Concept
    await ctx.writeMemory({
      content: 'Deadlock occurs when two or more threads are each waiting for the other to release a lock, creating a circular dependency that prevents progress',
    ...
    // Fact
    await ctx.writeMemory({
      content: 'Go channels are typed conduits for goroutine communication, they block on send when full and on receive when empty',
    ...
    // Reference
    await ctx.writeMemory({
      content: 'The POSIX threads (pthreads) API provides pthread_mutex_lock, pthread_mutex_unlock, and pthread_cond_wait for thread synchronization',
    ...
    // How-to
    await ctx.writeMemory({
      content: 'To avoid deadlocks, always acquire locks in a consistent global order, use timeout-based lock attempts, or employ lock-free data structures',
    ...
    // Insight
    await ctx.writeMemory({
      content: 'In practice, lock-free queues with CAS operations outperform mutex-based queues by 3-5x under high contention in multi-core systems',
    ...
    await ctx.writeMemory({
      content: 'A mutex allows multiple threads to access a resource simultaneously (WRONG - this is superseded)',
      tags: [KB_TAG, KB_DEFINITION_TAG],
      ownership: {
        domain: KB_DOMAIN_ID,
    ...
    console.log(`  Sections: Definitions=${hasDefinitions}, Facts=${hasFacts}, HowTos=${hasHowtos}`)
...
describe('Knowledge consolidation schedule (real)', () => {
  ...
  test('consolidation merges similar entries of the same classification', async () => {
    ...
    console.log(`[CONSOLIDATION] After: ${afterNonSuperseded.length} non-superseded, ${afterSuperseded.length} superseded`)
    ...
    if (afterSuperseded.length > 0) {
      ...
      const consolidated = consolidatedSearch.entries.find(m => {
        const attrs = m.domainAttributes[KB_DOMAIN_ID] as Record<string, unknown> | undefined
      ...
      console.log('[INFO] Consolidation did not merge — similarity may be below threshold with mock embeddings')
    }
...
describe('Search expansion and ask() (real)', () => {
  ...
  test('search finds entries and ask() synthesizes knowledge', async () => {
    ...
    // Populate knowledge
    await ctx.writeMemory({
      content: 'B-trees are self-balancing tree data structures that maintain sorted data and allow searches, sequential access, insertions, and deletions in O(log n) time',
      tags: [KB_TAG, KB_DEFINITION_TAG],
      ownership: {
        domain: KB_DOMAIN_ID,
        attributes: { classification: 'definition', superseded: false },
    ...
    await ctx.writeMemory({
      content: 'PostgreSQL uses B-trees as the default index type, which is optimal for equality and range queries on ordered data',
      tags: [KB_TAG, KB_FACT_TAG],
      ownership: {
        domain: KB_DOMAIN_ID,
        attributes: { classification: 'fact', superseded: false },
    ...
    await ctx.writeMemory({
      content: 'To create a partial index in PostgreSQL: CREATE INDEX idx_name ON table (column) WHERE condition — useful for filtering on frequently queried subsets',
      tags: [KB_TAG, KB_HOWTO_TAG],
      ownership: {
        domain: KB_DOMAIN_ID,
        attributes: { classification: 'how-to', superseded: false },
    ...
    await ctx.writeMemory({
      content: 'GIN (Generalized Inverted Index) indexes in PostgreSQL are best for full-text search, JSONB containment queries, and array operations',
      tags: [KB_TAG, KB_FACT_TAG],
      ownership: {
        domain: KB_DOMAIN_ID,
        attributes: { classification: 'fact', superseded: false },
    ...
    console.log(`[SEARCH] Found ${searchResult.entries.length} entries for "PostgreSQL indexing strategies"`)
    ...
    console.log(`  [ASSESSMENT] B-tree: ${mentionsBTree}, GIN: ${mentionsGIN}, Partial: ${mentionsPartial}`)

"""Phase 0.1 — tight adapter (rank 2, 4 layers, v/o only, α 16).

Repeats Phase 0 protocol with a constrained adapter to see if blast radius shrinks.
"""
...
from engine import BASE_MODEL, ingest_with_config  # noqa: E402
...
CONFIG = ROOT / "configs" / "phase0-1-lora.yaml"
ADAPTER = ROOT / "adapters" / "phase0_1"
RESULTS = ROOT / "notes" / "phase0_1-results.md"
...
def main() -> None:
    ...
    print(f"[2/4] Ingest — config={CONFIG.name}")
    ADAPTER.mkdir(parents=True, exist_ok=True)
    t0 = time.time()
    ingest_with_config(FACTS, ADAPTER, CONFIG)
    ...
    adapter_size = sum(f.stat().st_size for f in ADAPTER.glob("adapters.safetensors"))
    RESULTS.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        f"# Phase 0.1 — {verdict}",
        "",
        f"Config: `{CONFIG.relative_to(ROOT)}` — rank 2, 4 layers, v/o only, α 16 (scale 8), 150 iters, lr 5e-5.",
        "",
        f"- ingest_time: {ingest_s:.1f}s",
        f"- adapter size: {adapter_size / 1024:.1f} KB (vs Phase-0 20 MB)",
